...
import cl.vc.xroalpaca.actors.OrderTracker;
...
@Slf4j
public class OrderManagerBuyside extends AbstractActor {
    private static final int ACTORS_BUFFER_SIZE = 10_000;
    private final Channel channel;
    private final ConcurrentLinkedQueue<ActorRef> availableActors = new ConcurrentLinkedQueue<>();
    private final HashMap<String, ActorRef> orderTrackers = new HashMap<>();
    private ActorRef newActor;
    private RoutingMessage.OrderCancelReject.Builder reject = RoutingMessage.OrderCancelReject.newBuilder();

    private OrderManagerBuyside(Channel channel) {
        this.channel = channel;
        reject.setText("Repeated ID");

        while (availableActors.size() < ACTORS_BUFFER_SIZE) {
            ActorRef newActor = getContext().actorOf(OrderTrackerBuySide.props(channel));
    ...
    public static Props props(Channel channel) {
        return Props.create(OrderManagerBuyside.class, channel);
    }

    @Override
    public void preStart() {
        newActor = getContext().actorOf(new RoundRobinPool(1).props(OrderTracker.props(channel)));
    }
    ...
    private void notifyError(Exception e) {
        NotificationMessage.Notification notification = NotificationUtil
                .buildErrorNotification(MainApp.getSecurityExchange(),
